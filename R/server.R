shinyTandemServer <- function(input, output, session) {
  ...
  output$protExpect <- renderPlot({
    ...
    max.expect <- 0.01
    if (! is.na(result@used.parameters$`output, maximum valid expectation value`)){
      max.expect <- as.numeric(result@used.parameters$`output, maximum valid expectation value`)
    }
