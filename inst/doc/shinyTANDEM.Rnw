...
The graphical interface is launched with the function \texttt{shinyTANDEM()}, optional parameters can be passed to the shiny function. Specifically, the \texttt{port} parameter allows you to specify a given port. Launching the interface will open your default browser to shinyTANDEM homepage. From there, the first step is usually to load a dataset. The package comes with an example dataset. To load it, enter \texttt{result.Rda} and \texttt{result} in the R-data section, or \texttt{result.xml} in the xml section. 
